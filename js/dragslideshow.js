	 ...
	 */
	 DragSlideshow.prototype._initEvents = function() {
	 	...
	 	this.slides.forEach( function( slide ) {
			...
			if(slide.dataset.content!=="content-13"){
			...
			if (self.disabled==false){
				...
				if( self.isContent ) {
					...
					case 40:
						...
						if( !self.isFullscreen || slideshow.current>10 ) return;
						self._toggleContent( currentSlide );
						break;
					// right and left keys
					case 37:
	 ...
	 DragSlideshow.prototype._initDragDealer = function() {
	 	...
	 	this.dd = new Dragdealer( this.imgDragger, {
	 		...
	 		callback: function( x, y ) {
	 			...
	 			let currentStep = this.getStep()[0]-1; 

	 			if(currentStep==0){
	 				document.title = "Velta - Préface "
	 				player.isPlaying() ? player.pause() : false ; 
	 				header.setAttribute("hidden",""); 
	 				previous = currentStep; 
	 			}else if(currentStep>11){
	 				document.title = "Velta - Bientôt disponible "
	 				player.isPlaying() ? player.pause() : false ; 
	 				header.setAttribute("hidden",""); 
	 				previous = currentStep; 
	 				...
	 				document.title = "Velta - "+currentStep;
	 				header.removeAttribute("hidden");

	 				if (previous!=currentStep){
	 					player.skipTo(currentStep); 
	 					previous = currentStep; 
	 				}else{

	 					if (event.path[0].className!="content-switch" && event.path[0].className!="switch-right" && event.path[0].className!="switch-left"){
